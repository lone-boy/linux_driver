...
static long led_unlocked_ioctl(struct file *filp,unsigned int cmd,unsigned long arg)
{
    ...
    /* 自旋锁上锁 */
    spin_lock_irqsave(&gpioled.spinlock,flags);
    ...
    spin_unlock_irqrestore(&gpioled.spinlock,flags);
    return 0;
}
...
/* 定时回调函数 */
void led_timer_function(struct timer_list *timer){
    static bool on = 1;
    unsigned long flags;
...
static int __init led_init(void)
{
   ...
   led_gpio_init();
    ...
    /* 初始化定时器 */
    gpioled.timer.function = led_timer_function;
